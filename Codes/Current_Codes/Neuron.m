classdef Neuron < handle
    ...
    methods
        function self = Neuron(v,varargin)
            self.params = parse_inputs(v,varargin{:});
            self.index = indices();
    ...
    %% Algebraic Expressions
            ...
            % Insert parameters for all non integers
            
            % v(1) through v(9)
            % Na flux through NaP channel in soma using GHK
            m1alpha     = 1 ./ (v(1)*6 * (1 + exp(-((v(2)*0.143 * v_sa) + v(3)*5.67))));
            m1beta      = exp(-((v(2)*0.143 * v_sa) + v(3)*5.67)) ./ (v(1)*6 * (1 + exp(-((v(2)*0.143 * v_sa) + v(3)*5.67))));
            h1alpha     = v(4)*5.12e-8 * exp(-((v(5)*0.056 * v_sa) + v(6)*2.94));
            h1beta      = v(7)*1.6e-6 ./ (1 + exp(-(((v(8)*0.2 * v_sa)) + v(9)*8)));
            ...
            m8alpha     = v(10)*0.32 * ((-v_sa - v(11)*51.9) ./ (exp(-(v(12)*0.25 * v_sa + v(12)*v(11)*12.975)) - 1));
            m8beta      = v(13)*0.28 * ((v_sa + v(14)*24.89) ./ (exp(v(15)*0.2 * v_sa + v(15)*v(14)*4.978) - 1));
            h6alpha     = v(16)*0.128 * exp(-(v(17)*0.056 * v_sa + v(18)*2.94));
            h6beta      = v(19)*4 ./ (1 + exp(-(v(15)*0.2 * v_sa + v(20)*6)));
            ...
            m2alpha     = v(21)*0.016 * ((v_sa + v(22)*34.9) ./ (1 - exp(-((v(23)*0.2 * v_sa) + v(23)*v(22)*6.98))));
            m2beta      = v(24)*0.25 * exp(-((v(25)*0.025 * v_sa) + v(26)*1.25));
            ...
            m3alpha     = v(27)*0.02 * ((v_sa + v(28)*56.9) ./ (1 - exp(-((v(29)*0.1 * v_sa) + v(29)*v(28)*5.69))));
            m3beta      = v(30)*0.0175 * ((v_sa + v(31)*29.9) ./ (exp(((v(29)*0.1 * v_sa) + v(29)*v(31)*2.99)) - 1));
            h2alpha     = v(32)*0.016 * exp(-((v(33)*0.056 * v_sa) + v(34)*4.61));
            h2beta      = v(35)*0.5 ./ (1 + exp(-((v(36)*0.2 * v_sa) + v(37)*11.98)));
            ...
            m4alpha     = 1 ./ (v(38)*6 * (1 + exp(-((v(39)*0.143 * v_d) + v(40)*5.67))));
            m4beta      = exp(-((v(39)*0.143 * v_d) + v(40)*5.67)) ./ (v(38)*6 * (1 + exp(-((v(39)*0.143 * v_d) + v(40)*5.67))));
            h3alpha     = v(41)*5.12e-8 * exp(-((v(42)*0.056 * v_d) + v(43)*2.94));
            h3beta      = v(44)*1.6e-6 ./ (1 + exp(-(((v(45)*0.2 * v_d)) + v(46)*8)));
            ...
            m5alpha     = v(47)*0.5 ./ (1 + exp((v(48)*13.5 - K_e) / v(49)*1.42));
            m5beta      = v(47)*0.5 - m5alpha;
            h4alpha     = 1 ./ (v(50)*2000 * (1 + exp((K_e - v(51)*6.75) / v(52)*0.71)));
            h4beta      = v(53)*5e-4 - h4alpha;
            ...
            m6alpha     = v(54)*0.016 * ((v_d + v(55)*34.9) ./ (1 - exp(-((v(56)*0.2 * v_d) + v(56)*v(55)*6.98))));
            m6beta      = v(57)*0.25 * exp(-((v(58)*0.025 * v_d) + v(59)*1.25));
            ...
            m7alpha     = v(60)*0.02 * ((v_d + v(61)*56.9) ./ (1 - exp(-((v(62)*0.1 * v_d) + v(62)*v(61)*5.69))));
            m7beta      = v(63)*0.0175 * ((v_d + v(64)*29.9) ./ (exp(((v(62)*0.1 * v_d) + v(62)*v(64)*2.99)) - 1));
            h5alpha     = v(65)*0.016 * exp(-((v(66)*0.056 * v_d) + v(67)*4.61));
            h5beta      = v(68)*0.5 ./ (1 + exp(-((v(69)*0.2 * v_d) + v(70)*11.98)));
      ...
      %% Conservation equations                                                         
            ...
            du(idx.Buff_e, :)   = v(71)*p.Mu * K_e .* (v(72)*p.B0 - Buff_e) ./ (1 + exp(-((K_e - v(73)*5.5) ./ v(74)*1.09))) - (v(71)*p.Mu * Buff_e);
       ...
       function [Glu, J_K_NEtoSC, NO_n, O2] = shared(self, t, u)    %shared variables
            ...
            % Apply the same parameter variation here as in the buffer, see
            % the section labels as "% change in buffer for K+ in the
            % extracellular space" around line 234
            v = self.v;
            dKedt = 1/(p.Farad * p.fe) * (((p.As * J_K_tot_sa) / p.Vs)  + ((p.Ad * J_K_tot_d) / p.Vd)) - (v(71)*p.Mu * K_e .* (v(72)*p.B0 - Buff_e) ./ (1 + exp(-((K_e - v(73)*5.5) ./ v(74)*1.09))) - (v(71)*p.Mu * Buff_e));
...
function params = parse_inputs(v,varargin)
    parser = inputParser();
...
%     parser.addParameter('Imax', 0.013*4);  
    ...
    % NO pathway 
    parser.addParameter('m_c', 4*v(75));              % [-] Number of Ca2+ bound per calmodulin (approximated as parameter, originally an algebraic variable that changed from 3.999 to 4)
    
    parser.addParameter('K_mA', 650*v(76));           % [uM] - fit to Santucci2008
    parser.addParameter('K_mB', 2800*v(77));          % [uM] - fit to Santucci2008

    parser.addParameter('v_n', -0.04*v(78));          % [V] ; the neuronal membrane potential , assumed to be approx constant in this model
    parser.addParameter('G_M', 46000*v(79));          % [fS]! was 46 pS! ; the conductance of the NMDA channel to Ca2+ compaired  
    parser.addParameter('P_Ca_P_M', 3.6*v(80));       % [-] ; the relative conductance of the NMDA channel to Ca2+ compared to monovalent ions
    parser.addParameter('Ca_ex', 2e3*v(81));          % [microM] ; the external calcium concentration (in Comerford+David2008: 1.5 mM!)
    parser.addParameter('M', 1.3e5*v(82));            % [microM] ; the concentration of monovalent ions in the neuron

    parser.addParameter('n_NR2A', 0.63*v(83));        % [-] ; average number of NR2A NMDA receptors per synapse (Santucci2008)
    parser.addParameter('n_NR2B', 11*v(84));          % [-] ; average number of NR2B NMDA receptors per synapse (Santucci2008)
    parser.addParameter('V_max_NO_n', 4.22*v(85));    % [s^-1] ; maximum catalytic rate of NO production (Chen2006) - obtained from fig 6 & equ 17 & 18
    parser.addParameter('O2_n', 200*v(86));           % [uM] ; tissue O2 concentration in the neuron (M.E.)
    parser.addParameter('K_mO2_n', 243*v(87));        % [uM] ; Chen2006
    parser.addParameter('LArg_n', 100*v(88));         % [uM] ; 
    parser.addParameter('K_mArg_n', 1.5*v(89));       % [uM] ; 
    
    parser.addParameter('k_O2_n', 9.6e-6*v(90));      % [uM^-2 s^-1] ; % (Kavdia2002)
    parser.addParameter('x_nk', 25*v(91));            % [um] ;  (M.E.)
    parser.addParameter('D_cNO', 3300*v(92));         % [um^2 s^-1] ; Diffusion coefficient NO (Malinski1993)
    
    parser.addParameter('V_spine', 8e-8*v(93));       % [nL] ; volume of the neuronal dendritic spine Santucci2008
    parser.addParameter('k_ex', 1600*v(94));          % [s^-1] ; decay rate constant of internal calcium concentration Santucci2008
    parser.addParameter('Ca_rest', 0.1*v(95));        % [uM] ; resting calcium concentration (in Comerford+David2008: 2.830 mM; in Santucci2008P: 0.1 \muM)
    parser.addParameter('lambda_buf', 20*v(96));      % [-] ; buffer capacity Santucci2008

    parser.addParameter('V_maxNOS', 25e-3*v(97));     % [] ; M.E.
    parser.addParameter('K_actNOS', 9.27e-2*v(98));   % [uM] ; 
    parser.addParameter('mu2_n', 0.0167*v(99));       % [s^-1] ; rate constant at which the nNOS is deactivated Comerford2008
