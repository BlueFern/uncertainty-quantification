classdef SMCEC < handle
    ...
    methods
        function self = SMCEC(v,varargin)
...
function params = parse_inputs(v,varargin)
    ...
    % NO pathway
    parser.addParameter('D_cNO', 3300*v(1)); % [um^2 s^-1] ; Diffusion coefficient NO (Malinski1993)
    parser.addParameter('K_mArg_j', 1.5*v(2)); % [] ;
    parser.addParameter('K_mO2_j', 7.7*v(3)); % [] ; Chen2006
    parser.addParameter('k_dno', 0.01*v(4)); % [s^-1] ;
    parser.addParameter('K_m_mlcp', 5.5*v(5)); % [uM] ;
    parser.addParameter('V_NOj_max', 1.22*v(6)); % [s^-1] ; maximum catalytic rate of NO production (Chen2006) - obtained from fig 6 & equ 17 & 18
    parser.addParameter('O2_j', 200*v(7)); % [uM] ; O2 concentration in the EC (ME)
    parser.addParameter('LArg_j', 100*v(8)); % [uM] ;
    parser.addParameter('k_O2', 9.6e-6*v(9)); % [uM^-2 s^-1] ;
    parser.addParameter('W_0', 1.4*v(10)); % [Pa^-1] ; shear gating constant (Comerford2008)
    parser.addParameter('delta_wss', 2.86*v(11)); % [Pa] ; the membrane shear modulus (Comerford2008)
    parser.addParameter('k_1', 100*v(12)); % [s^{-1}] ;
    parser.addParameter('k1', 2e3*v(13)); % [uM^-1 s^-1] ;
    parser.addParameter('k2', 0.1*v(14)); % [s^-1] ;
    parser.addParameter('k3', 3*v(15)); % [uM^-1 s^-1] ;
    parser.addParameter('V_max_sGC', 0.8520*v(16)); % [] ;
    parser.addParameter('k_pde', 0.0195*v(17)); % [s^-1] ;
    parser.addParameter('C_4', 0.011*v(18)); % [s^{-1} microM^{-2}] ;
    parser.addParameter('K_m_pde', 2*v(19)); % [uM] ;
    parser.addParameter('gam_eNOS', 0.1*v(20)); % [-] ;
    parser.addParameter('mu2_j', 0.0167*v(21)); % [s^-1] ;
    parser.addParameter('K_dis', 9e-2*v(22)); % [uM s^-1] ;
    parser.addParameter('K_eNOS', 4.5e-1*v(23)); % [uM] ;    
    parser.addParameter('g_max', 0.06*v(24)); % [uM s^-1] ;    
    parser.addParameter('alp', 2*v(25)); % [-] ; zero shear open channel constant (Comerford2008); in Wiesner1997: alp = 3
    parser.addParameter('delta_p_L', 9.1e4*v(26)); % 9.1e4: ME
    parser.addParameter('x_ki', 25*v(27)); % [um]  (M.E.)
    parser.addParameter('x_ij', 3.75*v(28)); % [um]  (Kavdia2002)
