classdef Astrocyte < handle
    ...
    methods
        function self = Astrocyte(v,varargin)
...
%     parser.addParameter('v_6', 22e-3); %V
    ...
    parser.addParameter('x_nk', 25*v(2));            % [um] ;  (M.E.)
    parser.addParameter('x_ki', 25*v(3));            % [um] ;  (M.E.)
    parser.addParameter('k_O2_k', 9.6e-6*v(4));      % [uM^-2 s^-1] ;  (Kavdia2002)
    parser.addParameter('O2_k', 200*v(5));           % [uM] ;  (M.E.)
